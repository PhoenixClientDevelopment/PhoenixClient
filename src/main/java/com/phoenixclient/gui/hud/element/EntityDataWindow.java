...
public class EntityDataWindow extends GuiWindow {
    ...
    protected void drawWindow(GuiGraphics graphics, Vector mousePos) {
        ...
        if (label.get()) {
            DrawUtil.drawText(graphics, "Entity Data:", getPos().getAdded(2, 2), ColorUtil.HUD_LABEL);
            yOff += 12;
        }
        ...
        if (entity instanceof LivingEntity living) {
            ...
            DrawUtil.drawDualColorText(graphics, "\"" + entity.getName().getString() + "\"", " (" + MathUtil.roundDouble(living.getHealth(), 2) + "/" + MathUtil.roundDouble(living.getMaxHealth(),2) + ")", getPos().getAdded(2, 2 + yOff), Color.WHITE, healthColor);
        } else {
            DrawUtil.drawText(graphics, "\"" + entity.getName().getString() + "\"", getPos().getAdded(2, 2 + yOff), Color.WHITE);
        }
        yOff += 12;
        DrawUtil.drawText(graphics, " " + entity.getType().toShortString(), getPos().getAdded(2, 2 + yOff), Color.WHITE);
        yOff += 12;
        if (entity instanceof AbstractHorse horse) {
            ...
            DrawUtil.drawDualColorText(graphics, " Speed: ", speed + " m/s", getPos().getAdded(2, 2 + yOff), Color.WHITE, speedColor);
            yOff += 12;
            DrawUtil.drawDualColorText(graphics, " Jump: ", blockHeight + " m", getPos().getAdded(2, 2 + yOff), Color.WHITE, jumpColor);
            yOff += 12;
        }
        ...
        if (uuid != null) {
            ...
            DrawUtil.drawText(graphics, " Owner: " + this.currentOwnerName, getPos().getAdded(2, 2 + yOff), Color.WHITE);
            yOff += 12;
        }
